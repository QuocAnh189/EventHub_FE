...
const ProductGridItem = ({ product, index, isSlide }: any) => {
  ...
  return (
    <Wrapper className='card flex flex-col h-full' {...wrapperProps}>
      ...
      <NavLink
        className={`h6 !leading-[1.4] block max-w-[180px] transition hover:text-accent ${isSlide ? 'mb-3' : ''}`}
        to='/product-editor'
      >
        {product.name}
      </NavLink>
      ...
      <div className={`flex flex-col flex-1 ${isSlide ? 'gap-1 mt-1.5' : 'gap-2.5 mt-2.5'}`}>
        <p className='font-heading font-bold text-sm leading-[1.4] text-green'>Available : {product.in_stock || 0}</p>
        <p className='font-heading font-bold text-sm leading-[1.4] text-accent'>Already sold : {product.sold || 0}</p>
        {!isSlide && (
          <>
            <p className='font-heading font-bold text-sm leading-[1.4]'>
              Regular price : ${product.regular_price || 0}
            </p>
            <p className='font-heading font-bold text-sm leading-[1.4]'>Sale price : ${product.sale_price || 0}</p>
          </>
        )}
      </div>
      {!isSlide && (
